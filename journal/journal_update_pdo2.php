...
if (isset($_GET['article_id']) && !$_POST) {
  // prepare SQL query
  $sql = 'SELECT article_id, image_id, title, article FROM journal
		  WHERE article_id = ?';
  ...
  $sql = 'SELECT cat_id FROM image_cat_lookup
          WHERE article_id = ?';
  $stmt = $conn->prepare($sql);
...
if (isset($_POST['update'])) {
  ...
  if (!empty($_POST['image_id'])) {
	...
	$sql = 'UPDATE journal SET image_id = NULL, title = ?, article = ?
			WHERE article_id = ?';
	$stmt = $conn->prepare($sql);
	// execute query by passing array of variables
  ...
  $sql = 'DELETE FROM image_cat_lookup WHERE article_id = ?';
  $stmt = $conn->prepare($sql);
  ...
  if (isset($_POST['category']) && is_numeric($_POST['article_id'])) {
	...
	if ($values) {
	  $sql = 'INSERT INTO image_cat_lookup (article_id, cat_id)
...
<p><a href="../admin/journal_list_pdo.php">List all entries </a></p>
<?php 
if (isset($error)) {
